...
// let und;                                         // коробка є але вона пуста тому undefined
...
// console.log(arr[4]);                                   // Array йдуть в послідовному порядку
...
// console.log(typeof(answer));                              // ІНФОРМАЦІЯ ЯКА ПРИХОДИТЬ ВІД КОРИСТУВАЧА БУДЕ У ВИГЛЯДІ STRING
...
// console.log(typeof(answers));                         // тип данних буде object
...
// // &&(и)
// // Serhiy Tsubylskiy, [31.08.2022 13:09]
// // В цьому операторі 2 значення мають видати true

// // Serhiy Tsubylskiy, [31.08.2022 13:10]
// // Якщо 2 значення true то береться останнє

// // Serhiy Tsubylskiy, [31.08.2022 13:11]
// // Якщо 2 значення false то береться перше
...
// // 3) git clone і шлях репозиторію(силку яка знаходиться на github в Code) і папку куди буде все складатись
...
// // switch - завжди йде на СТРОГЕ порівняння на відміну від простих умов як вище де є < > (більше, менше)
...
// while (num <= 55) {          // while - поки в нас ця умова виконується ми будем робити якісь дії
//     console.log(num);
//     num++;
// }
...
for(let i = 1; i < 10; i++ ) {
    if(i===6) {
        ...
        // continue;                // оператор continue пропускає 6 і рухається далі не пририваючи цикл
...
// // defer - загружає скріпт в фоновому режимі, ніколи не блокує сторінку, виконується тоді коли наше дом дерево готове, якщо є декілька defer то загружається по черзі
...
// document.body.append(script);              //цей елемент існує лише в скріпті і шоб добавити його   // поміщаєм елемент в кінець себе
